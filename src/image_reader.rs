...
pub fn read_image_into_vec(filename: &str) -> Vec2D<(u8, u8, u8, u8)> {
    ...
    Vec2D::new(pixel_arr, w.try_into().unwrap(), h.try_into().unwrap())
}

pub fn make_image_from_vec(data: Vec2D<(u8, u8, u8, u8)>) -> image::RgbaImage {
    let data_flattened: Vec<u8> = data
        .data
        .iter()
    ...
    image::ImageBuffer::from_raw(
        data.w.try_into().unwrap(),
        data.h.try_into().unwrap(),
        data_flattened,
    )
    .unwrap()
}
